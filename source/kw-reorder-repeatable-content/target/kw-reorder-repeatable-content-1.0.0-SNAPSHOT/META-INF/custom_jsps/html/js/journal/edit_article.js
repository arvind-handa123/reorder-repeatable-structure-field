...
function updateFieldsDisplay() {
	YUI().use('node', function (Y) {
		...
		var input = Y.one("[name=_15__fieldsDisplay]");
...
function moveWrapper(clickedWrapper, direction) {
	...
	var getAllowIndex = clickedWrapper.getAttribute('data-allowIndex').split(",");
	var getNextWrapperAllowIndex = nextWrapper.getAttribute('data-allowIndex').split(",");
	
	if(AUI().one(clickedWrapper.getDOMNode()).ancestor('[data-fieldname^="Section_Order"]'))
	{
		var belowArray=['RegionalFocus','MostRead','Related'];
		var failedNextWrapper = false;
		var failedClickedWrapper = false;
		if(getAllowIndex.indexOf(nextWrapper.getAttribute('data-scopeIndex')) != -1){
			if (getNextWrapperAllowIndex.indexOf(clickedWrapper.getAttribute('data-scopeIndex')) != -1){
				clickedWrapper.swap(nextWrapper);
				setTimeout(function(){
					var sectionOrderArray = AUI().all('[data-fieldname^="Section_Order_Internal"]').getDOM();
					for(i = 0; i < sectionOrderArray.length; i++){
						AUI().one(sectionOrderArray[i]).removeAttribute('data-scopeIndex').setAttribute('data-scopeIndex',i);
					}
				},1000);
				...
				failedNextWrapper = true;
			}
		}
		else{
			failedClickedWrapper = true;
		}
		
		if(failedClickedWrapper){
			if (belowArray.indexOf(clickedWrapper.getAttribute('data-sectionName')) != -1){
				alert("The "+clickedWrapper.getAttribute('data-sectionName')+" section cannot be moved lower than position "+parseInt(parseInt(clickedWrapper.getAttribute('data-scopeIndex'))+1));
			}
			else{
				alert("The "+clickedWrapper.getAttribute('data-sectionName')+" section cannot be moved higher than position "+parseInt(parseInt(clickedWrapper.getAttribute('data-scopeIndex'))+1));
			}
		}
		if(failedNextWrapper){
			if (belowArray.indexOf(nextWrapper.getAttribute('data-sectionName')) != -1){
				alert("The "+nextWrapper.getAttribute('data-sectionName')+" section cannot be moved lower than position "+parseInt(parseInt(nextWrapper.getAttribute('data-scopeIndex'))+1));
			}
			else{
				alert("The "+nextWrapper.getAttribute('data-sectionName')+" section cannot be moved higher than position "+parseInt(parseInt(nextWrapper.getAttribute('data-scopeIndex'))+1));
			}
		}
	}
	else{
